...
# loading csv file with stock data and assign to a Data Frame
    df=pd.read_csv(file_name, index_col=0)
    print('\nThe top values:',df.head())
    return df
    
def printing_unique_tickers(data_frame):
# printing unique company symbols (ticker values)
...
# create a dta frame (with column 'close') for a ticker of our choice.
    df=data_frame
...
# modifying ticker df for seq_length columns (50 in this case)
    df=data_frame
...
# creating a Sequential model with 2 LSTM layers and 1 Dense layer.
    model = Sequential()
...
# plotting both true test data and predicted data in the same graph
